...
-- categories — admin-managed dynamic lists (material / vendor / cuisine)
...
create table if not exists public.raw_materials (
  ...
  material_type      varchar(20)   not null default 'INGREDIENT'
...
create table if not exists public.purchase_bills (
  ...
  photo_paths         text[]      not null default '{}',  -- multi-photo bills (PO receiving)
...
-- purchase_orders / purchase_order_lines — unified staff→admin workflow for
-- vendor POs and internal kitchen indents (see 0034).
...
-- categories + purchase-order RLS (policies reference the tenant helpers above).
do $$
declare t text;
begin
  foreach t in array array['categories','purchase_orders','purchase_order_lines'] loop
